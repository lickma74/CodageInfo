"""Schémas de présentation — restauration LPC pôles + zéros."""
...
def _boite(ax, cx, cy, w, h, texte):
    pad = 0.08
    ax.add_patch(
        FancyBboxPatch(
            (cx - w / 2, cy - h / 2), w, h,
            boxstyle=f"round,pad={pad},rounding_size=0.28",
            facecolor="white", edgecolor="black", linewidth=1.35, zorder=2,
        )
    )
    ax.text(cx, cy, texte, ha="center", va="center", fontsize=10, zorder=3, linespacing=1.25)
    return {
        "cx": cx, "cy": cy,
        "top": cy + h / 2 + pad, "bottom": cy - h / 2 - pad,
...
def _fleche(ax, x0, y0, x1, y1):
    ax.add_patch(
        FancyArrowPatch(
            ...
            linewidth=1.25, color="black", shrinkA=0, shrinkB=0, zorder=1,
        )
    )


def _chemin(ax, points):
    xs, ys = zip(*points)
...
def schema_processus(chemin):
    ...
    lpc = _boite(ax, 3.15, 11.85, w, h, "Pôles LPC + zéros\nA(z), B(z)")
    fine = _boite(ax, 7.55, 11.85, 3.7, h, "x[n] en continu")
    comp = _boite(ax, 3.15, 8.7, w, 1.55, "Warp θ → θ/α\nbande parole, aigus gardés")
    rec = _boite(ax, 3.15, 5.45, w, 1.45, "e = (A/B)·x\ny = (B'/A')·e")
    ...
    _chemin(ax, [(fine["cx"], fine["bottom"]), (fine["cx"], rec["cy"]), (rec["right"], rec["cy"])])
...
def schema_pipeline(chemin):
    ...
    b2 = _boite(ax, xs[2], y_haut, w, h, "Pôles + zéros\nA(z), B(z)")
    b3 = _boite(ax, xs[3], y_haut, w, h, "Warp θ → θ/α\naigus conservés")
    b4 = _boite(ax, xs[0], y_bas, w, h, "Filtres SOS\nA/B et B'/A'")
    b5 = _boite(ax, xs[1], y_bas, w, h, "e = (A/B)·x\ny = (B'/A')·e")
    ...
    _chemin(ax, [(b3["cx"], b3["bottom"]), (b3["cx"], y_coude),
                 (b4["cx"], y_coude), (b4["cx"], b4["top"])])
